...
// Constructeur
Grid::Grid(int r, int c) : rows(r), cols(c) { // on définit les colonnes et les lignes de notre grille
...
int Grid::countAliveNeighbors(int x, int y) const {
    ...
    for (int i = -1; i <= 1; ++i) {
        for (int j = -1; j <= 1; ++j) {
            ...
            // Cela relie le bord gauche au bord droit, et haut avec bas = avoir une grille torique style Pac-Man.
            int nx = (x + i + rows) % rows; 
            int ny = (y + j + cols) % cols;

            // Permet d'accèder directement à la cellule voisine (qui est forcément valide grâce au modulo)
...
// Applique les règles pour passer à l'état suivant (Parallélisme)
void Grid::applyRules(const Rules& rules) {
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            // countAliveNeighbors est "thread-safe" car elle est 'const' (lecture seule)
